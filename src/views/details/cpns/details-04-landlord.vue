...
const {landlordModule} = storeToRefs(useDetailStore()) 

</script>


<template>
  <div class="landlordModule">
    <detail-section  header="房东介绍" footer="查看房东主页">
      <template #main>
        <div class="intro">
          <div class="top">
                ...
                <div class="tags">
                  <template v-for="(item, index) in landlordModule?.hotelTags">
                    <div class="item" :style="{ color: item.tagText.color }">
                      ...
                      <span v-if="index !== landlordModule?.hotelTags.length - 1" class="divider">|</span>
                    </div>
                  </template>
                </div>
              </div>
